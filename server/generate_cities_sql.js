...
copartData.forEach(state => {
    state.locations.forEach(location => {
        if (location.name) {
            citySet.add(location.name.trim());
        }
    });
});
...
console.log(`Found ${cities.length} unique location names\n`);
...
const sqlQuery = `-- Insert ${cities.length} unique location names from Copart and IAAI
