/**
 * LEGACY: Old distance-based price calculation (kept for reference)
 * New logic uses POST /api/calculator instead.
 *
 * This file contains the original quote calculation logic that was based on:
 * - Geo API calls to calculate distance from yard to Poti, Georgia
 ...
 *
 * DO NOT USE IN NEW CODE - This is preserved only for historical reference
 * and potential fallback scenarios. All new quote calculations should use
 * the CalculatorService which calls POST /api/calculator.
 *
 * @deprecated Use ShippingQuoteService with CalculatorService instead
 */
...
interface LegacyQuoteComputationResult {
  distanceMiles: number;
  quotes: LegacyComputedQuote[];
}

/**
 * LEGACY: ShippingQuoteService
 *
 * This is the original implementation that computed quotes using:
 * - GeoLocatorService for geocoding yard locations
 * - Haversine formula for distance calculation
 * - Company pricing fields for quote computation
 *
 * @deprecated Preserved for reference only. Use the new calculator-based approach.
 */
export class LegacyShippingQuoteService {
  ...
  /**
   * LEGACY: Determine the approximate distance from the vehicle's yard to Poti, Georgia.
   *
   ...
   *
   * @deprecated Use calculator API instead
   */
  private getDistanceForYard(yardName: string, source: string): number {
    ...
    this.fastify.log.warn({ yardName, source }, 'LEGACY: Using default distance for unknown yard');
    return 8000;
  }

  /**
   * LEGACY: Compute great-circle distance between two coordinates in miles
  ...
  /**
   * LEGACY: Derive distance from the vehicle's yard to Poti, Georgia.
   *
   * Primary strategy:
   ...
   * @deprecated Use calculator API instead
   */
  async getDistanceForVehicle(vehicle: Vehicle): Promise<number> {
    ...
    try {
      ...
      if (anyFastify.redis && typeof anyFastify.redis.get === 'function') {
        try {
          ...
          this.fastify.log.error({ error, yardName }, 'LEGACY: Failed to read yard geocache from Redis');
        }
      }
      ...
      if (db && typeof db.execute === 'function') {
        try {
            ...
            if (lat != null && lon != null) {
              ...
              if (Number.isFinite(distance) && distance > 0) {
                ...
                try {
                  ...
                  this.fastify.log.error({ error, yardName }, 'LEGACY: Failed to update precomputed yard distance');
                }
                return rounded;
              }
            }
          ...
          this.fastify.log.error({ error, yardName }, 'LEGACY: Failed to read yards table for distance');
        }
      }
      ...
      if (!origin || origin.lat == null || origin.lon == null) {
        this.fastify.log.warn({ yardName }, 'LEGACY: GeoLocatorService returned no coordinates, falling back to default distance');
        return this.getDistanceForYard(yardName, vehicle.source);
      }
      ...
      if (anyFastify.redis && typeof anyFastify.redis.set === 'function') {
        try {
          ...
          this.fastify.log.error({ error, yardName }, 'LEGACY: Failed to write yard geocache to Redis');
        }
      }
      ...
      if (db && typeof db.execute === 'function') {
        try {
          ...
          this.fastify.log.error({ error, yardName }, 'LEGACY: Failed to upsert yard into yards table');
        }
      }
      ...
      if (!Number.isFinite(distance) || distance <= 0) {
        this.fastify.log.warn({ yardName }, 'LEGACY: Computed distance was invalid, falling back to default');
        return this.getDistanceForYard(yardName, vehicle.source);
      }

      return Math.round(distance);
    } catch (error) {
      this.fastify.log.error({ error, yardName }, 'LEGACY: Failed to compute distance using GeoLocatorService, falling back');
      return this.getDistanceForYard(yardName, vehicle.source);
    }
  }

  /**
   * LEGACY: Safely convert a value to a number for pricing calculations.
   *
  ...
  /**
   * LEGACY: Port coordinates lookup
   *
   * @deprecated Use calculator API instead
   */
  ...
  /**
   * LEGACY: Get distance from an auction branch address to selected port.
   *
  ...
  async getDistanceForAddress(address: string, source: string, port: string = 'poti_georgia'): Promise<number> {
    ...
    if (db && typeof db.execute === 'function') {
      try {
        ...
        this.fastify.log.error({ error, address }, 'LEGACY: Failed to read auction_branch_distances table');
      }
    }
    ...
    if (!origin || origin.lat == null || origin.lon == null) {
      this.fastify.log.warn({ address }, 'LEGACY: GeoLocatorService returned no coordinates for auction branch');
    ...
    if (db && typeof db.execute === 'function') {
      try {
        ...
        this.fastify.log.error({ error, address }, 'LEGACY: Failed to upsert auction_branch_distances');
      }
    }
  ...
  /**
   * LEGACY: Compute shipping quotes for all companies based on a given distance.
   *
  ...
  /**
   * LEGACY: Compute quotes for a vehicle across all companies.
   *
   ...
   * @deprecated Use calculator API instead
   */
  async computeQuotesForVehicle(vehicle: Vehicle, companies: Company[]): Promise<LegacyQuoteComputationResult> {
    ...
    const quotes: LegacyComputedQuote[] = [];
