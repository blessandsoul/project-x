...
import { SocialIconLink } from '@/lib/socialIcons';
...
const CompanyProfilePage = () => {
  ...
  // Locale-aware description with fallback chain
  const displayDescription = useMemo(() => {
    if (!company) return '';
    const locale = i18n.language;
    // Georgian locale
    if (locale === 'ka' && company.description_geo) return company.description_geo;
    // Russian locale
    if (locale === 'ru' && company.description_rus) return company.description_rus;
    // English or fallback: try eng -> geo -> rus -> empty
    return company.description_eng || company.description_geo || company.description_rus || '';
            ...
            <div className="space-y-6">
              ...
              <Card className="border-slate-200 bg-white shadow-sm">
                ...
                <CardContent className="space-y-4">
                  ...
                  {/* Social Networks */}
                  {(() => {
                    console.log('[CompanyProfile] social_links:', company.social_links);
                    const filtered = company.social_links?.filter((link) =>
                      link.platform &&
                      (link.platform === 'facebook' || link.platform === 'instagram') &&
                      (!link.link_type || link.link_type === 'social')
                    ) || [];
                    console.log('[CompanyProfile] filtered social links:', filtered);

                    return company.social_links && company.social_links.length > 0 && filtered.length > 0 ? (
                      <div className="pt-4 border-t border-slate-100 mt-4">
                        <div className="flex items-center justify-center gap-4">
                          {filtered.map((link, index) => (
                            <SocialIconLink
                              key={index}
                              platform={link.platform as 'facebook' | 'instagram'}
                              url={link.url}
                            />
                          ))}
                        </div>
