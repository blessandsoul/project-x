...
/**
 * IPhoneGridLayout - 2-column grid layout for iPhone
 * 
 * Matches /auction-listings mobile grid behavior: 2 cards per row.
 * Uses compact preview cards without action buttons.
 */
const IPhoneFeedLayout = memo(function IPhoneFeedLayout({ vehicles, loading, error, hideHeader }: LayoutProps) {
  if (loading) {
    return (
      <div className="w-full bg-slate-50 pointer-events-none select-none" aria-hidden="true">
        {/* App Header */}
        <div className="px-3 py-2.5 bg-white border-b border-slate-200">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <div className="text-[13px] font-bold text-slate-900">Auctions</div>
            </div>
            <div className="h-7 w-7 rounded-lg bg-slate-100 flex items-center justify-center">
              <Icon icon="mdi:magnify" className="h-4 w-4 text-slate-500" />
            </div>
        ...
        {/* Skeleton Grid - 2 columns like /auction-listings mobile */}
  ...
  return (
    <div className="w-full bg-slate-50" aria-hidden="true">
      {/* App Header - only show if not hidden */}
      {!hideHeader && (
        <div className="px-3 py-2.5 bg-white border-b border-slate-200 sticky top-0 z-20">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="h-7 w-7 rounded-lg bg-emerald-600 flex items-center justify-center">
                <Icon icon="mdi:car" className="h-4 w-4 text-white" />
      ...
      {/* 2-Column Grid - show all fetched vehicles for scrolling */}
      <div className="p-3 grid grid-cols-2 gap-2 pointer-events-none select-none">
        {vehicles.map((vehicle) => (
          <AuctionVehicleCard
            key={vehicle.id}
            item={vehicle}
